...
class PosterIngestionTest {
    ...
    fun `extracts one gig per line from the poster reply`() {
        ...
        expectThat(gigs).containsExactly(
            Gig(GigId(VenueId("some-venue"), GigUrl("https://example.com/post/1#gig-doom-night-2026-08-14")), GigTitle("Doom Night"), GigDate(2026, 8, 14), PosterUrl("https://example.com/poster.jpg"), GigDescription("Doom Night")),
            Gig(GigId(VenueId("some-venue"), GigUrl("https://example.com/post/1#gig-thrash-fest-2026-08-21")), GigTitle("Thrash Fest"), GigDate(2026, 8, 21), PosterUrl("https://example.com/poster.jpg"), GigDescription("Thrash Fest")),
        )
    }
    ...
    fun `ignores blank lines and stray text around the gig lines`() {
        ...
        expectThat(gigs.map { it.title }).containsExactly(GigTitle("Doom Night"))
    }
    ...
    fun `excludes The Dev's recurring karaoke night, keeping other gigs`() {
        ...
        expectThat(gigs.map { it.title }).containsExactly(GigTitle("Doom Night"), GigTitle("Thrash Fest"))
    }
    ...
    fun `does not exclude karaoke-titled gigs at other venues`() {
        ...
        expectThat(gigs.map { it.title }).containsExactly(GigTitle("Metal Karaoke Night"))
    }
