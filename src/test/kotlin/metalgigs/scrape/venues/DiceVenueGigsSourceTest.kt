...
class DiceVenueGigsSourceTest {
    ...
    fun `extracts gig events from Blondies Brewery Taproom's dice_fm venue page`() {
        assertScrapesGigs(
            ...
            size = 9,
            first = Gig(
                GigId(blondiesBreweryTaproom.id, GigUrl("https://dice.fm/event/2wqb7p-its-never-over-jeff-buckley-screening-12th-aug-blondies-brewery-london-tickets")),
    ...
    fun `extracts gig events from Blondies Bar's dice_fm venue page`() {
        assertScrapesGigs(
            ...
            size = 26,
            first = Gig(
                GigId(blondiesBar.id, GigUrl("https://dice.fm/event/av57g7-midweek-mayhem-4-pints-all-night-12th-aug-blondies-london-tickets")),
    ...
    fun `extracts gig events from Helgi's dice_fm venue page`() {
        assertScrapesGigs(
            ...
            size = 15,
            first = Gig(
                GigId(helgis.id, GigUrl("https://dice.fm/event/avrpa2-sceptocrypt-in-gods-way-cariad-14th-aug-helgis-london-tickets")),
    ...
    fun `extracts gig events from Barfly's dice_fm venue page`() {
        assertScrapesGigs(
            ...
            size = 24,
            first = Gig(
                GigId(barfly.id, GigUrl("https://dice.fm/event/xe37pm-propaganda-indie-club-night-at-barfly-15th-aug-barfly-camden-london-tickets")),
