...
class GigsStoreTest {
    ...
    fun `appends and reads back gig log entries of different kinds`() {
        val gig = Gig(id = GigId("Test Venue", "https://example.com/gigs/test-gig"), title = "Test Gig", date = LocalDate.of(2026, 8, 8), imageUrl = "https://example.com/images/test-gig.jpg")
    ...
    fun `reads back an observation written before the description existed, and one with it`() {
        val gig = Gig(id = GigId("Test Venue", "https://example.com/gigs/test-gig"), title = "Test Gig", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val recordedAt = Instant.parse("2026-08-01T12:00:00Z")
        val file = File.createTempFile("events", ".ndjson").apply { deleteOnExit() }
    ...
    fun `a gig whose description was never captured is not treated as changed`() {
        val gig = Gig(id = GigId("Test Venue", "https://example.com/gigs/never"), title = "Never", date = LocalDate.of(2026, 8, 9), imageUrl = "")
    ...
    fun `the description changing does not make a gig count as changed`() {
        val gig = Gig(id = GigId("Test Venue", "https://example.com/gigs/some-gig"), title = "Some Gig", date = LocalDate.of(2026, 8, 8), imageUrl = "")
    ...
    fun `projects the latest observation per gig, ignoring classification entries`() {
        val firstSeen = Gig(id = GigId("Test Venue", "https://example.com/gigs/some-gig"), title = "Some Gig", date = LocalDate.of(2026, 8, 8), imageUrl = "https://example.com/images/some-gig.jpg")
    ...
    fun `keeps separate gigs from different venues distinct`() {
        val gigA = Gig(id = GigId("Venue A", "https://example.com/gigs/same-slug"), title = "Gig A", date = LocalDate.of(2026, 8, 8), imageUrl = "https://example.com/images/gig-a.jpg")
        val gigB = Gig(id = GigId("Venue B", "https://example.com/gigs/same-slug"), title = "Gig B", date = LocalDate.of(2026, 8, 8), imageUrl = "https://example.com/images/gig-b.jpg")
    ...
    fun `treats a gig as new or changed if it's unseen or differs from its latest observation`() {
        val unseen = Gig(id = GigId("Test Venue", "https://example.com/gigs/unseen"), title = "Unseen Gig", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val unchangedGig = Gig(id = GigId("Test Venue", "https://example.com/gigs/unchanged"), title = "Unchanged Gig", date = LocalDate.of(2026, 8, 9), imageUrl = "")
        val soldOutBefore = Gig(id = GigId("Test Venue", "https://example.com/gigs/changed"), title = "Changed Gig", date = LocalDate.of(2026, 8, 10), imageUrl = "")
    ...
    fun `treats a gig as changed again if it reverts to a state seen earlier than its latest observation`() {
        val original = Gig(id = GigId("Test Venue", "https://example.com/gigs/reverting"), title = "Reverting Gig", date = LocalDate.of(2026, 8, 8), imageUrl = "")
    ...
    fun `derives last-scraped time per venue from the latest observation, ignoring venues never observed`() {
        val gigA1 = Gig(id = GigId("Venue A", "https://example.com/gigs/a1"), title = "Gig A1", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val gigA2 = Gig(id = GigId("Venue A", "https://example.com/gigs/a2"), title = "Gig A2", date = LocalDate.of(2026, 8, 9), imageUrl = "")
        val gigB = Gig(id = GigId("Venue B", "https://example.com/gigs/b"), title = "Gig B", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val events: List<LogEntry> = listOf(
    ...
    fun `detects an already-ingested poster by its gigs' shared source-url prefix`() {
        val gig = Gig(id = GigId("Some Venue", "https://example.com/post/1#gig-doom-night-2026-08-14"), title = "Doom Night", date = LocalDate.of(2026, 8, 14), imageUrl = "")
    ...
    fun `ignores a render entry when projecting gigs`() {
        val gig = Gig(id = GigId("Test Venue", "https://example.com/gigs/metal"), title = "Metal", date = LocalDate.of(2026, 8, 9), imageUrl = "")
        val recordedAt = Instant.parse("2026-07-01T00:00:00Z")
        val events: List<LogEntry> = listOf(
            GigObserved(gig, recordedAt),
            GigClassified(gig.id, recordedAt, Genre.Metal, ClassificationSource.LLM),
    ...
    fun `projects only gigs classified Metal, excluding never-classified ones`() {
        val neverClassified = Gig(id = GigId("Test Venue", "https://example.com/gigs/never-classified"), title = "Never Classified", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val metal = Gig(id = GigId("Test Venue", "https://example.com/gigs/metal"), title = "Metal", date = LocalDate.of(2026, 8, 9), imageUrl = "")
        val other = Gig(id = GigId("Test Venue", "https://example.com/gigs/other"), title = "Other", date = LocalDate.of(2026, 8, 10), imageUrl = "")
        val recordedAt = Instant.parse("2026-07-01T00:00:00Z")
        val events: List<LogEntry> = listOf(
    ...
    fun `computes classification status per gig, using the latest classification`() {
        val classified = Gig(id = GigId("Test Venue", "https://example.com/gigs/classified"), title = "Classified", date = LocalDate.of(2026, 8, 9), imageUrl = "")
        val reclassified = Gig(id = GigId("Test Venue", "https://example.com/gigs/reclassified"), title = "Reclassified", date = LocalDate.of(2026, 8, 10), imageUrl = "")
        val neverClassified = Gig(id = GigId("Test Venue", "https://example.com/gigs/never-classified"), title = "Never Classified", date = LocalDate.of(2026, 8, 11), imageUrl = "")
        val recordedAt = Instant.parse("2026-07-01T00:00:00Z")
        val events: List<LogEntry> = listOf(
    ...
    fun `a user override always beats the classifier's verdict`() {
        val overriddenToMetal = Gig(id = GigId("Test Venue", "https://example.com/gigs/overridden-to-metal"), title = "Overridden To Metal", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val overriddenToOther = Gig(id = GigId("Test Venue", "https://example.com/gigs/overridden-to-other"), title = "Overridden To Other", date = LocalDate.of(2026, 8, 9), imageUrl = "")
        val recordedAt = Instant.parse("2026-07-01T00:00:00Z")
        val events: List<LogEntry> = listOf(
    ...
    fun `treats any classification as already-classified, whoever made it`() {
        val classified = Gig(id = GigId("Test Venue", "https://example.com/gigs/classified"), title = "Classified", date = LocalDate.of(2026, 8, 8), imageUrl = "")
        val userOverridden = Gig(id = GigId("Test Venue", "https://example.com/gigs/user-overridden"), title = "User Overridden", date = LocalDate.of(2026, 8, 10), imageUrl = "")
        val neverClassified = Gig(id = GigId("Test Venue", "https://example.com/gigs/never-classified"), title = "Never Classified", date = LocalDate.of(2026, 8, 11), imageUrl = "")
        val recordedAt = Instant.parse("2026-07-01T00:00:00Z")
        val events: List<LogEntry> = listOf(
            GigObserved(classified, recordedAt),
