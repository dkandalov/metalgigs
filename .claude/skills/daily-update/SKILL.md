...
The script stages and commits only `events.ndjson`, `index.html`, `sitemap.xml` and `images/` — deliberately not `git add -A`, since the working tree also carries untracked local editor and tool config. Both the "is there anything to commit?" check and the commit itself name those paths, because a bare `git commit` takes the whole index: hand-written work left staged — which is what `git reset --soft` does with it — was committed by a scheduled run as its own on 2026-09-18. Naming the paths also leaves that work staged rather than taking it. If the user expects some other file to be published, that's a change to the script, not something to work around by staging it by hand.
